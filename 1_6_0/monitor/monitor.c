...
static void file_write_log(char *level, char* str_log) {
    ...
    bzero((char *) &pause_time_buffer,TIME_BUFFER_SIZE);
    ...
    if(0 == strcmp(level, "RMT"))
        ...
        sprintf((char *) &pause_time_buffer, "%s RMT: %s \n", time_buf, str_log);
    else if(0 == strcmp(level, "CLD"))
        //sprintf(&pause_time_buffer[0], "%s CLD: %s \n", time_buf, str_log);
        sprintf((char *) &pause_time_buffer, "%s CLD: %s \n", time_buf, str_log);
    else
        //sprintf(&pause_time_buffer[0], "%s MON: %s \n", time_buf, str_log);
        sprintf((char *) &pause_time_buffer, "%s MON: %s \n", time_buf, str_log);
    ...
    while (thread_run) {
        ...
        if ((serial_port_fd > 0)) {
            ...
            while ( read_ptr[msg_start_ptr]!=0xff || read_ptr[msg_start_ptr+1]!=0xff
                ...
                if(msg_start_ptr >= COMM_BUFFER_SIZE)
                {
                	msg_start_ptr = 0;
                	break;
                }
            ...
            if (strcasecmp(bat, "B0C") == 0) {
                logger_debug("Battery level < 25%%");
                battery_level = 5;
            } else if (strcasecmp(bat, "B1C") == 0) {
                logger_detailed("Battery level > 25%% and < 75%%");
                battery_level = 50;
            } else if (strcasecmp(bat, "B2C") == 0) {
                logger_detailed("Battery level > 75%% and < 100%%");
                battery_level = 90;
            } else if (strcasecmp(bat, "B0N") == 0) {
                logger_debug("Battery level < 25%% and not charging");
                battery_level = 5;
            } else if (strcasecmp(bat, "B1N") == 0) {
                logger_detailed("Battery level > 25%% and < 75%% and not charging");
                battery_level = 50;
            } else if (strcasecmp(bat, "B2N") == 0) {
                logger_detailed("Battery level > 75%% and < 100%% and not charging");
                battery_level = 90;
            } else if (strcasecmp(bat, "B2F") == 0) {
                logger_detailed("Battery level fully charged");
                battery_level = 100;
            } else if (strcasecmp(bat, "B0F") == 0) {
                logger_debug("Battery disconnected");
                battery_level = 100;
            }
            strcpy((char *) &battery_level_str, bat);
...
int get_operstate()
{
    ...
    readbyte = read(fd, (char *) & flag_buf, 15);
    if(readbyte > 0)
    {
...
int get_network_phy_state()
{
    ...
    if(total_states > 0 && total_states < 3 && recording == 0 && chargerON == 1)
    {
        ...
        {
            logger_info("System reboot because of ethernet flags missmatch: e:%d o:%d l:%d", eth, oper, link);
            //system("reboot");
        }
    }
    else
...
int consume_and_intrepret_UART_data() {
    ...
    if (serial_raw_read_last != serial_raw_read_cur) {
	    ...
	    char malform[7];
            memcpy((char *) &malform,serial_read_buf, 7);
...
char * get_battery_level_string() {
    ...
    bzero((char *) &battery_level_str, 16);
    int i;
    for (i=0;i<10;i++) {
...
char *get_command(char *cmd) {
    logger_detailed("Entering: %s",__FUNCTION__);
    bzero((char *) &command_str, 16);
    write_command_to_serial_port(cmd);
    sleep(1);
    return (char *)&command_str;
}

char *get_hw_version()
{
    logger_detailed("Entering: %s",__FUNCTION__);
    return (char *) &hw_version;
}

char *get_assignable()
{
    logger_detailed("Entering: %s",__FUNCTION__);
    return (char *) &assignable;
}

char *get_dvr_name()
{
    logger_detailed("Entering: %s",__FUNCTION__);
    return (char *) &dvr_name;
}
...
void start_capture(int cause) {
    ...
    char cmd[256];
    sprintf(cmd, "%s/%s >> %s/%s_%s 2>&1 &",
            ODI_BIN, ODI_CAPTURE, ODI_LOG, ODI_CAPTURE, ymd);
    logger_info("Capture command: %s/%s", ODI_BIN, ODI_CAPTURE);
    system(cmd);

    capture_pid = -1;
    current_found = check_current_file();
    if (!current_found) {
        logger_error("Current file not found, restarting gst_capture...");
        system("killall gst_capture");
        system(cmd);
        current_found = check_current_file();
        if (capture_pid == -1 || !current_found) {
...
int ttymx_action(char *command) {
    ...
    if (strcasecmp(command, "MUO") == 0) {
        if (capture_pid == -1) {
            ...
            logger_info("EVENT: Mute on");
            kill(capture_pid, SIGRTMIN);
            mute_on = 1;
	...
	while(1) {
	    if(capture_pid != -1) // && RecSizeChkState != 0)
		break;
        ...
        logger_info("EVENT: Mute off");
        kill(capture_pid, SIGRTMIN + 1);
        mute_on = 0;
        ...
        {
            if (!recording) {
                logger_debug("Killing gst_capture processes");
                system("killall gst_capture > /dev/null 2>&1");
                capture_pid = pid_find(ODI_CAPTURE);
                if (capture_pid == -1) {
	...
	pre_event_recording = 0;
        ...
        if ((!chargerON && battery_level < MINIMUM_BATTERY) || available_space() < MINIMUM_SPACE) {
            if (battery_level < MINIMUM_BATTERY) {
                logger_error("Stopping recording due to low battery.");
            } else {
        ...
        if(pre_event > 0) {
            logger_info("Start main record in pre_event case");
            kill(capture_pid, SIGCONT);
        } else {
        ...
        logger_info("EVENT: RCF requested: %d", capture_pid);
    ...
    if (RecSizeChkState == 2) {
        ...
        if(rec_time_cnt > 40 ) { // 2 sec to check
            ...
            if(retChkmkvSizeValue > planb_size) {
                ...
                char cmd[256];
                sprintf(cmd, "killall %s > /dev/null 2>&1", ODI_CAPTURE);
                system(cmd);
...
//    logger_info("Entering: %s ...", __FUNCTION__);
...
void send_aws_data(int data)
{
    ...
    bzero((char *) &aws_msg, sizeof(MVI_AWS_MSG_T));

    int aws_msgid = open_msg(MVI_AWS_MSGQ_KEY);
    if(aws_msgid < 0)
    {
        logger_info("Error invalid message queue");
        return;
    }
    aws_msg.header.moduleID = MVI_MONITOR_MODULE_ID;
    aws_msg.header.subType = MVI_MSG_CAM_DOCKED;
    aws_msg.cam_dock = data;
    strcpy(aws_msg.hwversion,(char *) &hw_version);
    send_msg(aws_msgid, (void *) &aws_msg, sizeof(MVI_AWS_MSG_T), 3);
}

//Send data to MQTT PUB
void send_mqtt_pub_data_response(int data)
{
    MVI_MQTT_PUB_MSG_T mqtt_pub_msg;
    bzero((char *) &mqtt_pub_msg, sizeof(MVI_MQTT_PUB_MSG_T));

    int pub_msgid = open_msg(MVI_MQTT_PUB_MSGQ_KEY);
    if(pub_msgid < 0)
    {
        logger_info("Error invalid message queue");
        return;
    }
    mqtt_pub_msg.header.subType = MVI_MSG_DATA_REQUEST;
    mqtt_pub_msg.header.moduleID = MVI_MONITOR_MODULE_ID;
    strcpy(mqtt_pub_msg.hwversion,(char *) &hw_version);
    mqtt_pub_msg.battery_level = data;
    send_msg(pub_msgid, (void *) &mqtt_pub_msg, sizeof(MVI_MQTT_PUB_MSG_T), 3);
}

//Send data to REMOTEM
void send_remotem_data_response(char *data)
{
    MVI_REMOTEM_MSG_T remotem_msg;
    bzero((char *) &remotem_msg, sizeof(MVI_REMOTEM_MSG_T));

    int remotem_msgid = open_msg(MVI_REMOTEM_MSGQ_KEY);
    if(remotem_msgid < 0)
    {
        logger_info("Error invalid message queue");
        return;
    }
    remotem_msg.header.subType = MVI_MSG_DATA_REQUEST;
    remotem_msg.header.moduleID = MVI_MONITOR_MODULE_ID;
    strcpy(remotem_msg.battery_str,get_battery_level_string());
    strcpy(remotem_msg.main_board,get_hw_version());
    strcpy(remotem_msg.assignable,get_assignable());
    strcpy(remotem_msg.dvr_name,get_dvr_name());

    send_msg(remotem_msgid, (void *) &remotem_msg, sizeof(MVI_REMOTEM_MSG_T), 3);
}

void monitor_msg_handler(MVI_MONITOR_MSG_T *Incoming)
{
    logger_detailed("Entering: %s ...", __FUNCTION__);
    switch(Incoming->header.subType)
    {
	    case MVI_MSG_DATA_REQUEST:
	    	if(battery_level == 0)
	    		get_battery_level();
	    	if(Incoming->header.moduleID == MVI_MQTT_PUB_MODULE_ID)
	    		send_mqtt_pub_data_response(battery_level);
	    	else if(Incoming->header.moduleID == MVI_REMOTEM_MODULE_ID)
	    		send_remotem_data_response(NULL);
	    	break;
	    case MVI_MSG_SERIAL_CMD:
	    	//logger_info("From: %s module receiving cmd: %s", module_list[Incoming->header.moduleID], Incoming->serial_cmd);
	    	write_command_to_serial_port(Incoming->serial_cmd);
	    	break;
	    default:
...
void *monitor_msg_task()
{
    logger_detailed("Entering: %s", __FUNCTION__);
    MVI_MONITOR_MSG_T monitor_msg;
    int monitor_msgid = create_msg(MVI_MONITOR_MSGQ_KEY);
    if(monitor_msgid < 0)
    {
        logger_info("Failed to open Monitor message");
        exit(0);
    }
    while(1)
    {
        recv_msg(monitor_msgid, (void *) &monitor_msg, sizeof(MVI_MONITOR_MSG_T), 5);
        monitor_msg_handler((MVI_MONITOR_MSG_T *) &monitor_msg);
        sleep(1);
    }
}

void *monitor_dog_bark_task()
{
    while(1) {
        send_dog_bark(MVI_MONITOR_MODULE_ID);
        sleep(1);
    }
}
...
int main(int argc, char* argv[]) 
{
    ...
    char cmd_buf[128];
    sprintf((char *) &cmd_buf, "echo \"%s \" > /odi/log/hw_version.txt", &hw_version);
    system((char *) &cmd_buf);
    //end of 10417
    ...
    logger_info("App version 1: %s, uCode version: %s, HW version: %s",
    ...
    pthread_t monitor_dog_id = -1, monitor_msg_id = -1;
    int result = -1;

    if(monitor_dog_id == -1)
    {
        result = pthread_create(&monitor_dog_id, NULL, monitor_dog_bark_task, NULL);
        if(result == 0)
            logger_detailed("Starting monitor dog bark thread.");
        else
            logger_info("Timer thread launch failed");
    }
    // Start thread for handle and send message 
    if(monitor_msg_id == -1)
    {
        result = pthread_create(&monitor_msg_id, NULL, monitor_msg_task, NULL);
        if(result == 0)
            logger_detailed("Starting monitor message thread.");
        else
            logger_info("Message thread launch failed");
    }
    ...
    while (!is_processing_done) {
...
#else
        if (read_button_states(&stealth_time_state, &snap_trace_state)) {
            logger_error("Failed to read button states.");
        }
#endif
        if ((serial_raw_read_cur != serial_raw_read_last)) {
        ...
        if (!camera_docked) {
            ...
            if (recording && free_space_cnt > 1200) {
                logger_detailed("Checking battery level and disk space...");
                get_battery_level();

                int dspace = available_space();
                logger_debug("Space available: %d MB, Battery level: %d%%",
                             dspace, battery_level);
                if (dspace < MINIMUM_SPACE || battery_level < MINIMUM_BATTERY) {
                    char cmd[256];
                    sprintf(cmd, "killall -s INT %s > /dev/null 2>&1", ODI_CAPTURE);
                    system(cmd);
                    if (battery_level < MINIMUM_BATTERY) {
                        logger_error("Stopping recording, low battery.");
                        logger_info("EVENT RECORD STOP [BAT]");
                        system("echo 4 > /odi/log/stopreason");
                    } else {
                        logger_error("Stopping recording, low disk space.");
            ...
            if (recording == 1) {
                ...
                free_space_cnt++;
                recording_counter++;
                if(RCO_buzz_enable > 0 && RCO_buzzing == 1 && !stealth_on)
                {
        ...
        // Checking for reset command (10 second delay)
        if (snap_trace_state && stealth_time_state && reset_counter > 120) {
            logger_info("####");
        ...
        if (power_off_cnt >= 10 && power_saver) {
            if ((recording == 0) && (camera_docked == 0)
                    && (chargerON == 0) && (pre_event == 0))
            {
            	send_generic_msg(MVI_MONITOR_MODULE_ID, MVI_MSG_HIBERNATE, HIBERNATE_ON);
            	sleep(1);
                logger_info("All conditions exist for sleep now...");
                //10420 Send sleep command to uC
                //
                ...
                sleep(1);
                send_generic_msg(MVI_MONITOR_MODULE_ID, MVI_MSG_HIBERNATE, HIBERNATE_OFF);
            }
            power_off_cnt = 0;
        }
        ...
        if (current_found && recording) size_check();
        ...
        if(capture_pid == -1 && camera_docked == 0)
        {
            if(pre_event > 0)
            {
                logger_info("Launch gst_capture for pre-event record: %d s", pre_event);
                system("/usr/local/bin/gst_capture &");
                sleep(2);
                capture_pid = pid_find(ODI_CAPTURE);
