...
static char hw_version[16];
static int got_uuid = 0;

static char pub_main_token[128];
static char pub_agency_uuid[128];
...
void send_monitor_data_request()
{
    ...
    monitor_msg.header.moduleID = MVI_MONITOR_MODULE_ID;
    send_msg(msgid, (void *) &monitor_msg, sizeof(MVI_MONITOR_MSG_T), 3);
}

void *mqtt_pub_dog_bark_task()
{
    while(1) {
        send_dog_bark(MVI_MQTT_PUB_MODULE_ID);
        sleep(1);
    }
}

void mqtt_pub_msg_handler(MVI_MQTT_PUB_MSG_T * Incoming)
{
    logger_detailed("Entering: %s ...", __FUNCTION__);
    switch(Incoming->header.subType)
    {
        case MVI_MSG_DATA_REQUEST:
            //Update data to global value;
	    logger_detailed("Hw version: %s, battery: %d",
        	Incoming->hwversion, Incoming->battery_level);
	    bat_level = Incoming->battery_level;
	    strcpy((char *) &hw_version, Incoming->hwversion);
            break;
        case MVI_MSG_SEND_TOKEN:
        	logger_cloud("%s: Main token: %s", __FUNCTION__, Incoming->main_token);
        	logger_cloud("%s: Agency uuid: %s", __FUNCTION__, Incoming->agency_uuid);

        	bzero((char *) &pub_main_token, 64);
        	bzero((char *) &pub_agency_uuid, 64);
            
        	strcpy((char *) &pub_main_token, Incoming->main_token);
        	strcpy((char *) &pub_agency_uuid, Incoming->agency_uuid);
        	got_uuid = 1;	//Only start PUB when receive token
        	break;
        default:
...
void *mqtt_pub_msg_task()
{
    logger_detailed("Entering: %s", __FUNCTION__);
    MVI_MQTT_PUB_MSG_T mqtt_pub_msg;
    int mqtt_pub_msgid = create_msg(MVI_MQTT_PUB_MSGQ_KEY);
    if(mqtt_pub_msgid < 0)
    {
        logger_info("Failed to open Mqtt pub message");
        exit(0);
    }
    while(1)
    {
        recv_msg(mqtt_pub_msgid, (void *) &mqtt_pub_msg, sizeof(MVI_MQTT_PUB_MSG_T), 5);
        mqtt_pub_msg_handler((MVI_MQTT_PUB_MSG_T *) &mqtt_pub_msg);
        sleep(1);
    }
}
...
void mqtt_pub_main_task()
{
    ...
    pthread_t mqtt_pub_dog_id = -1, mqtt_pub_msg_id = -1;
    if(mqtt_pub_dog_id == -1)
    {
        result = pthread_create(&mqtt_pub_dog_id, NULL, mqtt_pub_dog_bark_task, NULL);
        if(result == 0)
            logger_detailed("Starting mqtt pub dog bark thread.");
        else
            logger_info("Timer thread launch failed");
    }

    //start thread for module dog bark response
    if(mqtt_pub_msg_id == -1)
    {
        result = pthread_create(&mqtt_pub_msg_id, NULL, mqtt_pub_msg_task, NULL);
        if(result == 0)
            logger_detailed("Starting mqtt pub msg thread.");
        else
    ...
    while(1)
    {
    	...
    	if(checkhost() == 1) // && ready_pub == 1)
        {
            ...
            if(p_officer_uuid == NULL)
            {
                sprintf((char *) &l_msg, PUB_MQTT_NOUUID,
                    ...
                    (char *) &officer_name, (float) upload, (char *) &hw_version,
                    (float) bat_level/100, (float) disk,
	    ...
	    logger_detailed("PUB sleep in: %d seconds", mqtt_publish_freq);
	    send_monitor_data_request();
...
int mqtt_pub(char *pub_msg, int pub_msg_len)
{
    ...
    bzero((char *) &buf, 128);
    strcpy(p_uuid, "/");
    strcat(p_uuid, (char *) &pub_agency_uuid);
    strcat(p_uuid, "/dashboard");
    pub_test[2] = p_uuid;
    pub_test[4] = (char *) &pub_main_token;
    ...
    logger_detailed("topic: %s",pub_test[2]);
    logger_detailed("token: %s",pub_test[4]);
    logger_detailed("device: %s",pub_test[6]);
