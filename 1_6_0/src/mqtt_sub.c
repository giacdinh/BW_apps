                               ...
                               const struct mosquitto_message *message)
{
    ...
    logger_detailed("SUB receive message: %s", message->payload);
    ...
    if(strstr(message->payload, UNIT_LATEST_FW_TAG))
    {
        ...
        if(!strcmp((char*) SU_getVersion(),(char *) &server_fw))
            logger_detailed("Match firmware. NO update. Device fw: %s -- server fw: %s",
...
void *mqtt_sub_dog_bark_task()
{
    while(1) {
	send_dog_bark(MVI_MQTT_SUB_MODULE_ID);
        sleep(1);
    }
}
...
void mqtt_sub_main_task()
{
    ...
    pthread_t mqtt_sub_dog_id = -1;
    if(mqtt_sub_dog_id == -1)
    {
        result = pthread_create(&mqtt_sub_dog_id, NULL, mqtt_sub_dog_bark_task, NULL);
        if(result == 0)
            logger_detailed("Starting mqtt sub dog bark thread.");
        else
    ...
    while(1)
    {
        if(checkhost() == 1)
            ...
            logger_detailed("SUB:mqtt can't ping out. Unit sleep awhile then try again");
        sleep(30);
    }
    ...
    result = mqtt_token(unitID, unitID);
    if(result == BV_FAILURE)
    {
        logger_cloud("Failed to get MQTT token. Stop");
        return;
    ...
    while(1)
    {
        ...
        sleep(30);   //Wait for 30s before next listening
    }
    exit(0);
}
