...
void send_agency_uuid_to_pub(char *agency_uuid)
{
    MVI_MQTT_PUB_MSG_T pub_msg;
    bzero((char *) &pub_msg, sizeof(MVI_MQTT_PUB_MSG_T));
    ...
    pub_msg.header.subType = MVI_MSG_SEND_TOKEN;
    strcpy(pub_msg.agency_uuid, agency_uuid);
    strcpy(pub_msg.main_token, mqtt_main_token);
    send_msg(pub_msgid, (void *) &pub_msg, sizeof(MVI_MQTT_PUB_MSG_T), 3);
}
...
//    logger_cloud("SUB receive message: %s", message->payload);
    ...
    //send agency_uuid to mqtt_pub
    send_agency_uuid_to_pub((char *) &agency_uuid);
...
int mqtt_sub_init(char *token, char *topic, char *device)
{
    ...
    logger_info("SUB: Entering: %s ...", __FUNCTION__);
