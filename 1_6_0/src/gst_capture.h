...
#ifdef ODI_KERNEL
    #define ODI_AUDIO_DELAY 166666666
#else
...
const char* main_bin_description = "mfw_v4lsrc capture-mode=4 queue-size=10 name=video_src ! videorate force-fps=30/1 ! tee name=t ! queue name=image_queue max-size-buffers=0 ! jpegenc ! multifilesink name=image_multifile next-file=0 enable-last-buffer=FALSE async=FALSE sync=FALSE t. ! queue max-size-buffers=0 ! vpuenc codec=6 bitrate=6100000 framerate-nu=30 framerate-de=1 force-framerate=true name=h264_encoder ! interpipesink name=inter_video_sink_capture node-name=inter_video_sink_capture preroll-queue-len=0 sync=false async=false enable-last-buffer=false forward-eos=true forward-tag=true drop=false  alsasrc blocksize=4608 provide-clock=true latency-time=3000 ! audiorate ! queue max-size-buffers=0 ! volume name=volume ! mfw_mp3encoder bitrate=128 optmod=1 sample-rate=44100 ! delay name=audio_delay delay=0 !  interpipesink name=inter_audio_sink_capture node-name=inter_audio_sink_capture sync=false async=false enable-last-buffer=false forward-eos=true forward-tag=true preroll-queue-len=0 drop=false";
