...
int main(int argc, char **argv)
{
    ...
    printf("Update Media\n");
    if(strstr(filename, "mkv") && argc < 3)
    {
	printf("Please provide md5sum. aws_update text.mkv MD5SUM \n");
	exit(1);
    }
    ...
    {
	char *md5sum = argv[2];
	result = UpdateStatus(&main_token, filename, md5sum);
    }
    else
	result = UpdateStatus(&main_token, filename, NULL);

    if(result == BV_FAILURE)
    {
        printf("Error: Update Media failure\n");
        exit(1);
    } 
    else
	printf("Media Updated. It's OK to delete\n");
}
