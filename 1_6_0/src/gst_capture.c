...
static void file_write_log(char* level, char* str_log)
{
    ...
    char time_buf[20];
    strftime(time_buf, 20, "%Y%m%d", localtime(&now));
    char fileName[500];

    sprintf(fileName, "%s/%s_%s.log", ODI_LOG, ODI_STATUS_FILE, time_buf);
    FILE* fp = fopen(fileName, "a");
    strftime(time_buf, 17, "(%Y%m%d%H%M%S)", localtime(&now));
...
GstElement* gst_setting_up_pipeline()
{
    ...
    g_object_set(G_OBJECT(audio_delay), "delay", ODI_AUDIO_DELAY, NULL);
...
void gst_cap_signal_handler(int signo)
{
    ...
    if (signo == SIGINT)
    {
        if (g_pipe != NULL)
        {
            ...
            else
            {
                ...
                bin = gst_bin_get_by_name(GST_BIN(g_pipe), "sink_bin");
                gst_detach_trigger(0);
                gst_detach_trigger(1);
                gst_bin_remove(GST_BIN(g_pipe), bin);
    ...
    if (signo == SIGCONT)
    {
        logger_info("SIGCONT signal received: Triggering pre-event recording");
    ...
    if (signo == SIGQUIT)
    {
        logger_info("SIGQUIT signal received: Stopping pre-event recording");
...
void gst_detach_trigger(unsigned int nbin)
{
    ...
    logger_debug("detaching triggers: %s - %s", video_trigger_bin[nbin], audio_trigger_bin[nbin]);
    video_bin = gst_bin_get_by_name(GST_BIN(g_pipe), video_trigger_bin[nbin]);
    audio_bin = gst_bin_get_by_name(GST_BIN(g_pipe), audio_trigger_bin[nbin]);

    gst_bin_remove(GST_BIN(g_pipe), video_bin);
    gst_bin_remove(GST_BIN(g_pipe), audio_bin);

    GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(g_pipe), GST_DEBUG_GRAPH_SHOW_NON_DEFAULT_PARAMS, "gst_pipeline_trigger_detached");

    logger_debug("exit: detaching sink");
}
...
void gst_attach_trigger(unsigned int nbin)
{
    ...
    logger_debug("attaching triggers: %s - %s", video_trigger_bin[nbin], audio_trigger_bin[nbin]);

    if (!nbin)
    {
        video_bin = gst_parse_bin_from_description(video_pretrigger_1, FALSE, NULL);
        audio_bin = gst_parse_bin_from_description(audio_pretrigger_1, FALSE, NULL);
    }
    else
    {
        video_bin = gst_parse_bin_from_description(video_pretrigger_2, FALSE, NULL);
        audio_bin = gst_parse_bin_from_description(audio_pretrigger_2, FALSE, NULL);
    }
    ...
    gst_bin_add(GST_BIN(g_pipe), video_bin);
    gst_bin_add(GST_BIN(g_pipe), audio_bin);
    ...
    gst_element_set_state(video_bin, GST_STATE_PLAYING);
...
void gst_attach_sink()
{
    ...
    bin = gst_parse_bin_from_description(sink_bin, FALSE, NULL);
    gst_element_set_name(GST_ELEMENT(bin), "sink_bin");
    gst_bin_add(GST_BIN(g_pipe), bin);
...
int main(int argc, char* argv[])
{
    ...
    logger_info("RidgeRun version: 1.1.12_v2");
    ...
    // wait till pipeline state is changed
    {
        ...
        apending = GST_STATE_PLAYING;
        GstClockTime timeout = 1e8;//10 sec
